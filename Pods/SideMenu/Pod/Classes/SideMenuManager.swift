...
open class SideMenuManager : NSObject {
    ...
    /// Dura@objc tion of the animation when the menu is dismissed without gestures. Default is 0.35 seconds.
    open static var menuAnimationDismissDuration: Double = 0.35
    
    /// Duration of the remaining ani@objc mation when the menu is partially dismissed with gestures. Default is 0.35 seconds.
    open static var menuAnimationCompleteGestureDuration: Double = 0.35
    
    /// Amount to fade the existing vie@objc w controller when the menu is presented. Default is 0 for no fade. Set to 1 to fade completely.
