...
open class SideMenuTransition: UIPercentDrivenInteractiveTransition {
    ...
    @objc func handlePresentMenuRightScreenEdge(_ edge: UIScreenEdgePanGestureRecognizer) {
        SideMenuTransition.presentDirection = .right
        handlePresentMenuPan(edge)
    }
    
    @objc func handlePresentMenuPan(_ pan: UIPanGestureRecognizer) {
        if SideMenuTransition.activeGesture == nil {
            SideMenuTransition.activeGesture = pan
        } else if pan != SideMenuTransition.activeGesture {
            pan.isEnabled = false
            pan.isEnabled = true
            return
        } else if pan.state != .began && pan.state != .changed {
            SideMenuTransition.activeGesture = nil
        }
        
        // how much distance have we panned in reference to the parent view?
        guard let view = SideMenuTransition.presentingViewControllerForMenu?.view ?? pan.view else {
            return
        }
        ...
        if !SideMenuTransition.singleton.interactive {
            if translation.x == 0 {
            ...
            if let menuViewController = SideMenuTransition.viewControllerForMenu, let visibleViewController = SideMenuTransition.visibleViewController {
                SideMenuTransition.singleton.interactive = true
        ...
        let distance = translation.x / SideMenuTransition.menuWidth
        // now lets deal with different states that the gesture recognizer sends
        switch (pan.state) {
        case .began, .changed:
            if pan is UIScreenEdgePanGestureRecognizer {
                SideMenuTransition.singleton.update(min(distance * direction, 1))
            } else if distance > 0 && SideMenuTransition.presentDirection == .right && SideMenuManager.menuLeftNavigationController != nil {
                SideMenuTransition.presentDirection = .left
                SideMenuTransition.switchMenus = true
        ...
        default:
            SideMenuTransition.singleton.interactive = false
            view.transform = .identity
            ...
            if velocity >= 100 || velocity >= -50 && abs(distance) >= 0.5 {
                ...
                if ProcessInfo().operatingSystemVersion.majorVersion == 8 && SideMenuTransition.singleton.percentComplete > 1 - CGFloat.ulpOfOne {
                    SideMenuTransition.singleton.update(0.9999)
                }
                SideMenuTransition.singleton.finish()
            } else {
                SideMenuTransition.singleton.cancel()
            }
        }
    }
